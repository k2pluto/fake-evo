...
import { type EvolutionSetupData } from '../../common/types'
...
export interface ConnectInfo {
  ...
  setupData: EvolutionSetupData
}

async function connectEvolution(username: string, evolutionEntryUrl: string, vendor: string): Promise<ConnectInfo> {
...
export interface LobbyTableResponse {
  configs: Record<string, LobbyTableInfo>
  ws: WebSocket
}
...
export async function connectHonorlinkCrawler(username: string) {
  console.log(`connectHonorlinkCrawler username : ${username}`)
  ...
  return await connectEvolution(username, evolutionEntryUrl, 'honorlink')
}
...
export async function connectSwixCrawler(username: string) {
  ...
  return await connectEvolution(username, gameUrl, 'swix')
}
...
export async function connectUniongameCrawler(username: string) {
  console.log(`connectUniongameCrawler username : ${username}`)
  ...
  return await connectEvolution(username, gameUrl, 'uniongame')
}
