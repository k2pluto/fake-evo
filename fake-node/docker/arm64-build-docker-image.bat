...
docker buildx build --platform linux/arm64 -f ./dockerfiles/arm64-server-base.Dockerfile -t arm64-server-base .

cd ..

docker buildx build --platform linux/arm64 --build-arg ENV=%2 --build-arg STAGE_ENV=%2 -f ./docker/dockerfiles/%1.Dockerfile -t %1:%2 .
