...
async fn handle_request(
  ...
  println!("Path and query: {}", req.uri());
  println!("Path and query: {}", req.uri().to_string());
  println!("Path and query: {}", req.uri().path());
  ...
  let video_url = format!("wss://{}{}", login_data.streamHost2, req.uri());

  println!("Connecting to video url: {}", video_url);
  ...
  tokio::spawn(async move {
    match hyper::upgrade::on(&mut req).await {
      Ok(upgraded) => {
        ...
        if let Err(e) = handle_websocket_connection(ws_stream, video_url).await
        {
...
async fn handle_websocket_connection(
  ...
  let (video_socket, _) = connect_async(video_url)
    .await
